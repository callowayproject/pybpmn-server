...
class Cron(ICron):
    ...
    async def start_timers(self):
        ...
        try:
            ...
            events_list = await self.definitions.find_events(query)
            ...
            for item in items_list:
                time_due = item.get("timeDue")
                if time_due:
                    self.logger.log(f"...checking timer: {time_due}")
                    self.schedule_item(item)
        except Exception as exc:
            print(exc)

    async def item_timer_expired(self, item_id: str):
        await self.server.engine.invoke({"items.id": item_id}, None)

    async def process_timer_expired(self, event: Any):
        await self.definitions.update_timer(event.model_name)
        # referenceDateTime might need update
        event.reference_date_time = datetime.now().timestamp() * 1000
        self.schedule_process(event)
        await self.server.engine.start(event.model_name, None, None, event.element_id)
    ...
    def schedule_process(self, entry: Any):
        ...
        if delay is not None:
            ...
            print(f"scheduling process {entry.model_name} delayed by {delay} seconds, scheduled at: {schedule_at}")
